...
async def fetch_feed(source: RSSSource) -> List[Dict[str, Any]]:
    """Fetch and parse a single RSS feed (non-blocking)"""
    logger.info(f"Fetching feed: {source.name} ({source.url})")

    try:
        # Run feedparser in a thread executor to avoid blocking the event loop
        ...
        if feed.bozo:  # Check if feed has errors
            logger.warning(
                f"Feed {source.name} has parsing errors: {feed.bozo_exception}"
            )
        ...
        for entry in feed.entries:
            ...
            guid = (
                entry.get("id")
                or entry.get("link")
                or generate_content_hash(entry.get("title", ""), entry.get("link", ""))
            )
...
async def store_entries(entries: List[Dict[str, Any]]) -> int:
    ...
    logger.info(
        f"Stored {new_count} new entries (skipped {len(entries) - new_count} duplicates)"
    )
    return new_count
...
async def fetch_all_feeds():
    ...
    logger.info(
        f"RSS fetch completed: {total_processed} sources processed, {total_failed} failed, {total_new_entries} new entries"
    )
